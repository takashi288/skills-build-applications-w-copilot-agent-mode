...
app.get('/api/users/', async (req, res) => {
  const users = await safeQuery(async () => User.find({}).lean().exec(), fallbackUsers);
  res.json(users);
});

app.get('/api/teams/', async (req, res) => {
  const teams = await safeQuery(async () => Team.find({}).lean().exec(), fallbackTeams);
  res.json(teams);
});

app.get('/api/activities/', async (req, res) => {
  const activities = await safeQuery(async () => Activity.find({}).lean().exec(), fallbackActivities);
  res.json(activities);
});

app.get('/api/leaderboard/', async (req, res) => {
  const leaderboard = await safeQuery(async () => LeaderboardEntry.find({}).sort({ rank: 1 }).lean().exec(), fallbackLeaderboard);
  res.json(leaderboard);
});

app.get('/api/workouts/', async (req, res) => {
  const workouts = await safeQuery(async () => Workout.find({}).lean().exec(), fallbackWorkouts);
  res.json(workouts);
});
